...
When the software-team workflow applies, load and follow `$solweaver`.
...
parallel work, as authorization for the mandatory bounded team workflow.

Solweaver has one execution contract: team execution. Every task must dispatch
at least one bounded implementation worker while Sol remains the parent
orchestrator. Small, low-risk, and single-file work is not exempt. If the user
requires parent-only work or forbids subagents, stop before implementation and
explain that Solweaver cannot satisfy that request without changing workflows.
...
Solweaver has one assurance contract: final-strict. Every task performs parent
verification at every checkpoint, records one coherent cumulative assurance
unit, and targets one fresh reviewer call at the declared final boundary. Give
the unit a stable `ASSURANCE_UNIT_ID`, keep a
...
Final-strict applies to every task, including auth, authorization, secrets,
tenant isolation, money, data integrity, migrations, destructive behavior,
concurrency, public APIs, production-critical paths, and wide architectural
refactors. It requires a fresh read-only `solweaver_reviewer` verdict after
